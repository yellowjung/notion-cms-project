...
- 수락 기준과 완료 조건 정의
- **API 연동 및 비즈니스 로직 구현 Task는 아래 테스트 절차를 반드시 따름:**
  1. **구현 전**: Task 파일 내 `## 테스트 체크리스트`에 Playwright MCP 시나리오 먼저 작성
  2. **구현 중**: 각 세부 단계 완료 시마다 관련 시나리오 즉시 실행하여 검증
  3. **구현 후**: 전체 시나리오 재실행 → **모두 통과 확인 후에만** Task 완료 처리
- **테스트 도구: 반드시 Playwright MCP만 사용** (단위 테스트, 수동 확인 등으로 대체 불가)
